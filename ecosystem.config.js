...
module.exports = {
  apps: [
    ...
    // API Gateway servisi
    {
      ...
      env: {
        ...
        API_PORT: '4000',
        DATABASE_URL:
          'postgresql://ai_chat_user:ai_chat_password@localhost:5400/ai_chat_db?schema=public',
  ...
  deploy: {
    production: {
      ...
      'post-deploy':
        'bun install --frozen-lockfile && bun run build && pm2 reload ecosystem.config.js --env production',
      'pre-setup': '',
    },
    staging: {
      user: 'deploy',
      ...
      'post-deploy':
        'bun install --frozen-lockfile && bun run build && pm2 reload ecosystem.config.js --env staging',
      'pre-setup': '',
    },
  },
};
