...
>
> After following this file, you will be able to open Grafana and see _live_ charts of request latency, token usage, and model performance while chatting via the web UI.
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!parseBody.success) {
        return reply
          .code(400)
          .send({ error: 'Invalid message data', details: parseBody.error.format() });
      }
      ...
      const response = await runChatCompletion(context, userMessage, orchestratorOptions, {
        model: chosenModel,
      ...
      if (response.usage) {
        ...
        if (typeof completionTokens === 'number') {
          chatCompletionTokensTotal.inc(
            { model: chosenModel, type: 'completion' },
            completionTokens,
          );
        }
      }
