...
export const OrgAnalyticsPage: React.FC = () => {
  ...
  const softPercent: number | null = useMemo(() => {
    ...
    return Math.min(
      100,
      Math.round((quota.usageInWindowTokens / quota.monthlySoftLimitTokens) * 100),
    );
  }, [quota]);
        ...
        <Box mb={1}>
          {quota.hardLimitExceeded ? (
            <Alert severity="error" variant="outlined">
              This organization has exceeded its <strong>hard token limit</strong> in the selected
              window. New usage may require upgrading the plan or extending limits.
            </Alert>
          ) : quota.softLimitExceeded ? (
            <Alert severity="warning" variant="outlined">
              This organization has exceeded its <strong>soft token limit</strong> in the selected
              window. Consider upgrading the plan or optimizing usage.
            </Alert>
          ) : showNearSoftLimit ? (
            <Alert severity="info" variant="outlined">
              This organization has used approximately {softPercent}% of its soft token limit in the
              selected window.
            </Alert>
          ) : null}
        </Box>
        ...
        >
          <CardContent>
            ...
            <Typography variant="h6">
              {loading ? <CircularProgress size={18} /> : formatNumber(totalPromptTokens)}
            </Typography>
            <Typography variant="caption" color="text.secondary">
