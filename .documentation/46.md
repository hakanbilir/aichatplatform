...
export async function fetchBillingPlans(): Promise<{ plans: BillingPlanDto[] }> {
  return apiRequest<{ plans: BillingPlanDto[] }>('/billing/plans', { method: 'GET' });
}
...
export const OrgBillingPage: React.FC = () => {
  ...
  const [usageSummary, setUsageSummary] = useState<{ tokens: number; requests: number } | null>(
    null,
  );
            ...
            <CardContent sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              ...
              {currentPlan?.limits?.monthly_token_limit && usageSummary && (
                <Typography variant="caption" color="text.secondary">
                  ...
                  {Number(currentPlan.limits.monthly_token_limit).toLocaleString()} tokens this
                  month.
                </Typography>
              )}
            </CardContent>
                  ...
                  >
                    <CardContent sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                      ...
                      {plan.limits?.monthly_token_limit && (
                        <Typography variant="caption" color="text.secondary">
                          Up to {Number(plan.limits.monthly_token_limit).toLocaleString()} tokens /
                          month.
                        </Typography>
                      )}
