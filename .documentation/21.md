...
export function parseOrgHardQuotaExceededError(payload: any): OrgHardQuotaExceededError | null {
  ...
  const plan: OrgPlan | null =
    ...
    ['FREE', 'PRO', 'ENTERPRISE', 'CUSTOM'].includes(payload.plan)
...
export const ChatPage: React.FC = () => {
  ...
  const { sending, quotaError, sendMessage } = useChatStream({
    ...
    >
      ...
      <Box display="flex" alignItems="flex-end" gap={1}>
        <TextField
          ...
          placeholder={
            quotaError
              ? 'Hard quota limit reached – new messages are blocked.'
              : 'Send a message...'
          }
          disabled={inputDisabled}
        />
        <IconButton color="primary" onClick={handleSend} disabled={inputDisabled || !input.trim()}>
          <SendIcon />
        </IconButton>
      </Box>
    </Box>
