...
toolExecutionDurationSeconds.labels(tool.name, ctx.orgId || 'none', 'true').observe(durationSec);
```
...
toolExecutionDurationSeconds.labels(tool.name, ctx.orgId || 'none', 'false').observe(durationSec);
```
