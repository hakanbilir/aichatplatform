...
export function streamWithRouting(req: ChatCompletionRequest): AsyncIterableIterator<any> {
  const provider = resolveProvider(req.modelProvider);
          ...
          {models.map((m) => (
            ...
            >
              ...
              <Box display="flex" flexWrap="wrap" gap={0.5} mt={0.5}>
                ...
                {m.contextWindow && <Chip size="small" label={`Context: ${m.contextWindow}`} />}
                {m.maxOutputTokens && <Chip size="small" label={`Max out: ${m.maxOutputTokens}`} />}
