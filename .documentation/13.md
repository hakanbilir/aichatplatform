...
export default async function orgRoutes(app: FastifyInstance, _opts: FastifyPluginOptions) {
  ...
  app.patch('/orgs/:id', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    if (!parseBody.success) {
      return reply
        .code(400)
        .send({ error: 'Invalid org update', details: parseBody.error.format() });
    }
  ...
  app.post('/orgs/:id/members', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    if (!parseBody.success) {
      return reply
        .code(400)
        .send({ error: 'Invalid member data', details: parseBody.error.format() });
    }
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      if (!parseBody.success) {
        return reply
          .code(400)
          .send({ error: 'Invalid member update', details: parseBody.error.format() });
      }
