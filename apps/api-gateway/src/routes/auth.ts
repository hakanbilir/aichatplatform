...
export default async function authRoutes(app: FastifyInstance, _opts: FastifyPluginOptions) {
  // Generate dummy hash for timing attack prevention using the configured cost factor
  // This ensures that the timing for checking a non-existent user matches the timing for a real user
  const dummyHash = await hashPassword('dummy_password_protection_timing_attack_prevention');
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const orgSlugBase = orgDisplayName
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '');
    ...
    async (request, reply) => {
      ...
      return reply.send({
        ...
        activeOrg: activeOrg
          ...
          : null,
        organizations: user.orgMemberships.map(
          (m: { org: { id: string; name: string; slug: string }; role: string }) => ({
            id: m.org.id,
            name: m.org.name,
            slug: m.org.slug,
            role: m.role,
  ...
  app.get('/api/v1/auth/me', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const activeOrg =
      user.orgMemberships.find((m: { orgId: string }) => m.orgId === payload.orgId)?.org ?? null;
