...
export default async function sharingRoutes(app: FastifyInstance, _opts: FastifyPluginOptions) {
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      if (bodyParsed.data.expiresAt !== undefined) {
        updateData.expiresAt = bodyParsed.data.expiresAt
          ? new Date(bodyParsed.data.expiresAt)
          : null;
      }

      // Update conversation metadata if anonymize or hiddenMessageIds changed
      if (
        bodyParsed.data.anonymize !== undefined ||
        bodyParsed.data.hiddenMessageIds !== undefined
      ) {
        ...
        if (bodyParsed.data.anonymize !== undefined)
          shareConfig.anonymize = bodyParsed.data.anonymize;
        if (bodyParsed.data.hiddenMessageIds !== undefined)
          shareConfig.hiddenMessageIds = bodyParsed.data.hiddenMessageIds;
  ...
  app.post('/public/conversations/:slug', async (request, reply) => {
    ...
    if (!bodyParsed.success) {
      return reply
        .code(400)
        .send({ error: request.i18n.t('errors.invalidBody'), details: bodyParsed.error.format() });
    }
