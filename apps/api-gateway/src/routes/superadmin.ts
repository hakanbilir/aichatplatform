...
export default async function superadminRoutes(app: FastifyInstance, _opts: FastifyPluginOptions) {
  // ============================================
    ...
    return reply.send({
      users: users.map(
        (u: {
          ...
          createdAt: Date;
          updatedAt: Date;
          _count: { orgMemberships: number; conversations: number };
        }) => ({
          id: u.id,
          email: u.email,
          name: u.name,
    ...
    async (request, reply) => {
      ...
      const user = await prisma.user.update({
        ...
        data: {
          ...
          ...(updateData.customInstructions !== undefined && {
  ...
  app.patch(
    '/superadmin/organizations/:orgId/plan',
    { preHandler: [app.authenticate] },
    async (request, reply) => {
      const payload = request.user as JwtPayload;
      await assertSuperadmin(payload, request.i18n);
  ...
  app.post(
    '/superadmin/organizations/assign-free-subscription',
    { preHandler: [app.authenticate] },
    async (request, reply) => {
      const payload = request.user as JwtPayload;
      await assertSuperadmin(payload, request.i18n);
