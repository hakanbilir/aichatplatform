...
export const ChatPage: React.FC = () => {
  ...
  const {
    ...
    sendMessage,
    updateSettings,
    // setConversation is not used directly
  } = useChat(conversationId);

  const { models } = useModels(conversation?.orgId);
  ...
  // Sync local settings with conversation when loaded
  useEffect(() => {
    if (conversation) {
      ...
      // Force sync if not dirty, effectively resetting to conversation values when switching conversation
      // We assume if dirty is false, we should sync.
      if (!dirty) {
         setModel(nextModel);
  ...
  // Reset dirty state when conversation changes
  useEffect(() => {
    setDirty(false);
  }, [conversationId]);
  ...
  const handleSend = async (content: string) => {
    await sendMessage(content, { model, temperature, topP });
  };
