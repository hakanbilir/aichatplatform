...
export const ChatPage: React.FC = () => {
  ...
  // Removed local conversation state management in favor of useChat
  ...
  const {
    ...
    onConversationUpdated: (updated) => {
      // Sync local settings state if conversation updates from outside (e.g. initial load)
      if (!dirty) {
        if (updated.model) setModel(updated.model);
        if (updated.temperature !== undefined) setTemperature(clampTemperature(updated.temperature));
        if (updated.topP !== undefined) setTopP(clampTopP(updated.topP));
      }
    },
  ...
  // Removed manual conversation loading useEffect as useChat handles it.
