...
export const ChatPage: React.FC = () => {
  ...
  // useChat hook manages conversation state, streaming, and settings
  const {
    conversation,
    streaming,
    streamingText,
    ...
    onConversationCreated: (id) => {
      const event = new CustomEvent('conversation-created', { detail: id });
  ...
  useEffect(() => {
    ...
    const handleCreated = async (_e: Event) => {
      if (!token) return;
      try {
        await createNewConversation(t('conversation.new'));
      } catch {
        // ignore
      }
    };
      ...
      >
        ...
        <Box width={160} px={1}>
          ...
          <Slider
            ...
            max={2}
            step={0.1}
            onChange={(_, val) => settings.updateTemperature(val as number)}
          />
        </Box>
        ...
        <Box width={130} px={1}>
          ...
          <Slider
            ...
            max={1}
            step={0.05}
            onChange={(_, val) => settings.updateTopP(val as number)}
          />
        </Box>
        ...
        <Box display="flex" alignItems="center" gap={1}>
          ...
          <Tooltip title={t('settings.resetSettings')}>
            <span>
              <IconButton size="small" onClick={settings.reset} disabled={!conversation}>
                <RestartAltIcon fontSize="small" />
              </IconButton>
            </span>
          </Tooltip>
          <Tooltip title={settings.dirty ? t('settings.saveSettings') : t('settings.settingsUpToDate')}>
            <span>
              <IconButton
                size="small"
                color={settings.dirty ? 'primary' : 'default'}
                onClick={settings.save}
                disabled={!settings.dirty || settings.saving || !conversationId}
              >
                <SaveIcon fontSize="small" />
              </IconButton>
            </span>
          </Tooltip>
          {settings.savedAt && !settings.dirty && (
            <Chip
              size="small"
