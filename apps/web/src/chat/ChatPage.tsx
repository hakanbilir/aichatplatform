...
export const ChatPage: React.FC = () => {
  ...
  const {
    ...
    streamingText,
    usage,
    // error, // could display error toast
    model,
    temperature,
    topP,
    sendMessage,
  ...
  const handleSend = async (content: string) => {
      // images handling can be added here later by extending MessageInput
  ...
  const handleSaveSettings = async () => {
    ...
    setSaving(true);
    try {
      await updateSettings({ model, temperature, topP });
      setDirty(false);
      setSavedAt(Date.now());
    } finally {
      setSaving(false);
  ...
  return (
    <Box display="flex" flexDirection="column" flex={1}>
      ...
      <Box display="flex" alignItems="center" gap={0.5} px={2} pb={0.5}>
        ...
        {isLoading && (
            <IconButton size="small" onClick={stop} color="error" title={t('conversation.stop')}>
