...
setInterval(async () => {
  try {
    ...
    if (processed > 0) {
      console.log(`Processed ${processed} dataset ingestion jobs`);
    }
  } catch (err) {
...
// Training Run Worker
// Eğitim çalıştırma çalışanı
setInterval(async () => {
  try {
    const { processTrainingRunBatch } = await import('./trainingRunWorker');
    const processed = await processTrainingRunBatch(5);
    if (processed > 0) {
