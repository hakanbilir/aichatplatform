...
const searchRegex = /      \} else if \(conversation\.userId !== payload\.userId\) \{\s+\/\/ Should not happen due to OR conditions in query, but defensive coding\s+return reply\.code\(403\)\.send\(\{ error: request\.i18n\.t\('errors\.forbidden'\) \}\);\s+\}/g;
