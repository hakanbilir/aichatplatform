...
Your task is to design and implement a comprehensive, production-ready user interface with the following requirements: 1. Design System & Visual Style
...
• Overall mood: modern, clean, professional, high-contrast, and easy to scan. 2. Interactivity & Micro-Interactions
...
• Use micro-interactions to convey state and feedback (loading, success, error, disabled). 3. Responsiveness & Layout
...
• Avoid hover-only interactions that don’t translate to touch. 4. Information Architecture & Components
...
• Use icons and color coding to indicate status (success, warning, error, info). 5. Usability & Accessibility
• The UI must be:
...
• Provide clear error states and empty states (when there is no data). 6. Technical & Implementation Requirements (for Code Generation)
...
• Organized into logical folders (e.g., components/, layouts/, hooks/, theme/, etc., if relevant). 7. Interaction Scenarios
...
• No data available.
• Error fetching data.
• Successful data refresh. 8. Output Expectations
