...
**Learning:** Even when using an external `StreamStore` for high-frequency data, dispatching actions (like `TOKEN`) to a `useReducer` that returns a _new_ state object (reference inequality) for every event will still force the consuming component (`ChatPage`) to re-render 50+ times/sec, negating the benefits of the store.
**Action:** In `useReducer`, ensure that high-frequency actions (like `TOKEN`) return the _existing_ state object (reference equality) if the relevant state properties (like `status`) haven't actually changed. This allows React to bail out of re-renders.
